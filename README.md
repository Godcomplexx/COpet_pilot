...
> Animation) и рендер каждого экрана вынесены из `app_main.c` в модули `modes/`
> и `ui/`; переходы состояний покрыты host-тестами (`test/host/`). `app_main.c`
> теперь тонкий слой интеграции железа и главного цикла.
> Пользовательское меню в том же стиле показывает только готовые `FOCUS`,
> `ANIMATION` и `SETTINGS`; незавершённые функции скрыты до реализации.
